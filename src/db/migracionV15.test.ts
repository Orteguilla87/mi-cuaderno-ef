...
describe('migración v14 → v15 sobre una base con datos', () => {
  it('sube el esquema, rellena los campos nuevos y crea las filas que faltaban', async () => {
    ...
    await db.open()
    expect(ESQUEMA_ACTUAL).toBe(20)

    // — unidades: valores por defecto y criterios con su ciclo dentro —
    const u5 = await db.unidades.get('u5')
    // v19 le puso `etapa`; v20 pasó `nivel`→`niveles` y `pesoTrimestre`→`pesosPorNivel`.
    expect(u5).toMatchObject({ etapa: 'primaria', computa: true, trimestre: 2 })
    expect(u5!.niveles).toEqual([5])
    expect((u5 as { pesosPorNivel: Record<number, number> }).pesosPorNivel).toEqual({ 5: 0 })
