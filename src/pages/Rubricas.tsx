...
import { HojaImportarRubrica, type PlanRubrica } from '../components/HojaImportarRubrica'
...
export function Rubricas() {
  ...
  async function crearDesdeTabla(plan: PlanRubrica) {
    // Una sola transacción: o entra la rúbrica entera con sus niveles y sus
    // indicadores, o no entra ninguna. Una rúbrica a medias no es recuperable
    // desde la UI y el usuario no tendría forma de saber por dónde se cortó.
    const id = await db.transaction('rw', db.rubricas, async () => {
      const id = await crearRubrica(titulo || 'Rúbrica sin título')
...
/**
 * Vuelca el plan confirmado en el preview sobre una rúbrica recién creada.
 *
 * Un peso sin reconocer entra como 0, que en el editor y en `filasIniciales`
 * significa «reparto equitativo con sus hermanas», no «esta fila no cuenta».
 * Los descriptores vacíos —los de relleno ya vaciados por el parser— no se
 * guardan: la vista de calificación solo enseña la descripción si existe.
 */
async function aplicarPlan(rubricaId: string, plan: PlanRubrica) {
  const niveles: NivelRubrica[] = plan.niveles.map((n) => ({
    id: nuevoId(),
    etiqueta: n.etiqueta,
    valor: n.valor,
  }))
  const criterios: CriterioRubrica[] = plan.filas.map((f) => ({
    id: nuevoId(),
    titulo: f.titulo,
    pesoPct: f.pesoPct ?? 0,
    codigo: f.codigo,
    ...
    descripciones: Object.fromEntries(
      f.descripciones.map((texto, i) => [niveles[i]?.id, texto]).filter(([, t]) => t),
    ),
  }))
